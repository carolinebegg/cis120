...
let rec tree_height (t: 'a tree) : int = 
  begin match t with
    | Empty -> 0
    | Node (ln, h, rn) -> 
      let lh = tree_height ln in
      let rh = tree_height rn in
      1 + max lh rh 
  end  
...
let rec is_perfect (t: 'a tree) : bool =
  begin match t with
    | Empty -> true
    | Node (ln, h, rn) -> tree_height ln = tree_height rn 
                          && is_perfect ln && is_perfect rn
  end
...
let test () : bool =
  let ex_tree = Node (Node 
                        (Node 
                          (Empty, 5, Empty), 
                        52, Node (Empty, 2, Empty)),
                      19, Node 
                    (Node (Empty, 4, Empty), 
                    16, Node (Empty, 1, Empty))) in
  is_perfect ex_tree
...
let rec tree_max (t: 'a tree) : 'a =
  begin match t with
    ...
    | Node (Empty, h, Empty) -> h
    | Node (Empty, h, rn) -> max h (tree_max rn)
    | Node (ln, h, Empty) -> max h (tree_max ln)
    | Node (ln, h, rn) -> 
      let mxv = max (tree_max ln) (tree_max rn) in
      max h mxv
  end
...
let rec lookup (x: 'a) (t: 'a tree) : bool =
  begin match t with
    | Empty -> false
    | Node (ln, h, rn) -> 
      if x > h then lookup x rn 
      else if x < h then lookup x ln 
      else x = h
  end
...
let rec inorder (t: 'a tree) : 'a list =
  begin match t with
    | Empty -> []
    | Node (ln, h, rn) -> inorder ln @ [h] @ inorder rn
  end
...
let rec insert (x: 'a) (t: 'a tree) : 'a tree =
  begin match t with
    ...
    | Node (ln, h, rn) -> 
      if x = h then t 
      else if x < h then Node (insert x ln, h, rn) 
      else Node (ln, h, insert x rn)
  end
...
let tree_of_list (l: 'a list) : 'a tree =
  fold (fun h1 t1 -> insert h1 t1 ) Empty l
...
let rec delete (x: 'a) (t: 'a tree) : 'a tree =
  begin match t with
    ...
    | Node (ln, h, rn) -> 
      if x = h then
        begin match (ln, rn) with
          | (Empty, Empty) -> Empty
          | (Node _, Empty) -> ln
          | (Empty, Node _) -> rn
          | _ -> let mx = tree_max ln in
            Node (delete mx ln, mx, rn)
        end
      else if x < h then Node (delete x ln, h, rn)
      else Node (ln, h, delete x rn)
  end
