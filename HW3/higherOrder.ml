...
let rec assoc (key: 'k) (l: ('k * 'v) list) : 'v =
  ...
  | (k1, v1) :: tail -> if key = k1 then v1 else assoc key tail
  end
...
let capitalize (l: string list) : string list =
  begin match l with
  | [] -> []
  | h1 :: t1 -> String.uppercase_ascii h1 :: transform String.uppercase_ascii t1
  end
...
let list_length (l: 'a list) : int =
  fold (fun x count -> 1 + count) 0 l

let test () : bool =
  list_length ["hi";] = 1
;; run_test "list length: one list item" test

let test () : bool =
  list_length [1; 2] = 2
;; run_test "list length: two list items" test

let test () : bool =
  list_length [] = 0
;; run_test "list length: empty list" test
...
let for_all (pred: 'a -> bool) (l: 'a list) : bool =
  fold (fun h1 t1 -> pred h1 && t1) true l
...
let exists (pred: 'a -> bool) (l: 'a list) : bool =
  fold (fun h1 t1 -> pred h1 || t1) false l 
...
let ho_filter (pred: 'a -> bool) (l: 'a list) : 'a list =
  fold (fun h1 t1 -> if pred h1 then h1 :: t1 else t1) [] l
