...
let make_controller (v: 'a) : 'a value_controller =
  let value = ref v in
  let contlist = ref [] in
  { add_change_listener = (fun (newcl: ('a -> unit)) -> 
      contlist := !contlist @ [newcl]);
    get_value = (fun () -> !value);
    change_value = (fun (nv: 'a) -> (value := nv; 
        (List.iter (fun x -> x nv) (!contlist))));
  }
  ...
  let w = 
  {
    repaint = (fun (g: Gctx.gctx) -> 
      ...
      let checkoff() = 
        ...
        Gctx.draw_line g (0, 20) (20, 0);
      in
  
      if vc.get_value() then checkoff() else empty_cb(); 
      l.repaint (Gctx.translate g (25,0)));
    handle = (fun (g:Gctx.gctx) (e:Gctx.event) -> 
      if Gctx.event_type e = MouseDown 
        then vc.change_value (not (vc.get_value())) else()
    );
    size = (fun () -> (20 + fst (l.size()), max 20 (snd (l.size()))));
  } in
  w, vc
...
(* TODO: In Task 6 you may choose to add a radio_button widget, a
  ...
  let slider (value: int) (s: string) : widget * int value_controller = 
    let vc = make_controller value in
    let l = fst (label s) in
    let w = 
    {
      repaint = (fun (g: Gctx.gctx) -> 
        Gctx.draw_rect g (0, 0) (255, 35);
        Gctx.fill_rect g (0, 0) (vc.get_value(), 35));
      handle = (fun (g:Gctx.gctx) (e:Gctx.event) -> 
        if Gctx.event_type e = MouseDrag || Gctx.event_type e = MouseDown
          then vc.change_value (fst (Gctx.event_pos e g)) else()
      );
      size = (fun () -> (255, 35));
    } in
    hpair (hpair l (space (10, 0))) w, vc