...
(** A type for line thickness *)
type thickness = {t: int}

let thin : thickness = {t = 2}
let thick : thickness = {t = 8}
...
(** Produce a new Gctx.t with a different pen color *)
let with_thickness (g: gctx) (t: thickness) : gctx =
  { g with thickness = t }
...
let set_graphics_state (gc: gctx) : unit =
  let c = gc.color in
  let pw = gc.thickness in
  Graphics.set_color (Graphics.rgb c.r c.g c.b);
  Graphics.set_line_width pw.t
...
let draw_points (g: gctx) (pl: position list) : unit =
  List.iter (fun p -> draw_point g p) pl
...
let draw_rect (g: gctx) (p1: position) ((w, h): dimension) : unit =
  set_graphics_state g;
  let (x, y) = ocaml_coords g p1 in (* convert original position to ocaml 
    coordinates*)
